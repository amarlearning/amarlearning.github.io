...
Instead of every developer managing the Infrastructure code, with GitOps, everything is stored centrally ina. git repo and the env is always synced with what's defined in the repo and this means the git repo becomes the single source of truth which makes it easier to manage the infrastructure.

### **Increasing Security**

with GitOps, we now don’t have to give everyone in the team who needs to change something on the Infrastructure or in the k8s cluster direct access to execute the changes because it’s the CD pipeline that deploys the changes not individual developers from the laptop.
